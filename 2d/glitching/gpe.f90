...
program main
    ...
    ! .TRUE.  => Calculate angular momentum's change over OMEGA (angular velocity of cranking)
    ! .FALSE. => Calculate normally (without changing angular velocity of cranking)
    n_vortex = 1
    imag_pot_mode = 6
    real_pot_mode = 5
    if (.false.) then
        write (*, *) "Calculation initiated."
        ! Output file which contains (Cranking velocity / Dimensionless Chemical Potential / Chemical Potential in MeV / Circulation A / Circulation B)
        open (12, file="quantities.txt", action="write")
        ! Output file which contains (energy change over OMEGA)
